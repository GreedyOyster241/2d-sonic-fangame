...
func _physics_process(delta):
	...
	if abs(motion.x) > 10.0:
		target_offset = clamp(motion.x / 5.0, -max_offset, max_offset)
	else:
		target_offset = 0.0
...
# --- Crouch + Spindash CHARGE (Down on floor, almost stopped) ---
	...
	var speed_charge = 0
	spindashing = false
	if grounded and Input.is_action_pressed("Down") and abs(velocity.x) < 10.0:
		#DUCK
		print("Spindashing..?")
		canjump = false
	...
	if spindashing and is_on_floor() and not Input.is_action_just_pressed("Down"):
		print("Release!")
		motion.x = direction * speed_charge
